...
def conHandler(channel, data):
	...
	u = observer(X0_rec, V0, DU, DU_p, U0, D2U0, theta)

	retMsg = positionSim_t()
	retMsg.u = u
	lcm.publish("controlCommand",retMsg.encode())

def observer(X0_rec, V0, DU, DU_p, U0, D2U0, theta):
	global Xhat, X0, u, vd
	...
	# Control law part generates the control 
	# input corresponding to u in equation 3 and 4
