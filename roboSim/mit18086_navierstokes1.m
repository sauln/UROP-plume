function [X,Y,Ue,Ve,Q,P]=mit18086_navierstokes1(lx,ly,nx,ny)
...
% Ue = velocities placed on vertical cell interfaces
% P = pressure in the cell midpoints
% Ve = velocities on horizontal cell interfaces
% Q = stream function on cell corners
% X = defined on line 65.. I think it's just some numbers to help matlab index everything
% Y = 
...
%       
      ...
      %clf, 
%       figure(1),
%       contourf(avg(y),avg(x),P,20,'w-'), hold on
%       contour(y,x,Q,20,'k-');
%       colorbar
...
V_x_matrix=Ue(1:end-1,1:end-1);
V_y_matrix=Ve(1:end-1,1:end-1);
c0=max(max(max(V_x_matrix)),max(max(V_y_matrix)))
V_x_matrix=70*V_x_matrix/c0;
V_y_matrix=70*V_y_matrix/c0;

myP=flipud(P);
myQ=flipud(Q);
%figure(11),hold on
%      contourf(avg(y),avg(x),70*myP/c0,100), %hold on%,'w-'
      %contour(y,x,70*myQ/c0,20,'k-');
%      colorbar
      
      %quiver(y,x,(Ve./Len),(Ue./Len),.4,'y-')
%       axis equal, axis([0 ly 0 lx])
%title('')

%figure(12),hold on
%contourf(avg(y),avg(x),log(sqrt(V_x_matrix.^2+V_x_matrix.^2)+1),20)
%contourf(avg(y),avg(x),(sqrt(V_x_matrix.^2+V_x_matrix.^2)),20)
% colorbar
%title('')  
%axis equal, axis([0 ly 0 lx])

%figure(13),hold on
%h=pcolor(avg(y),avg(x),(sqrt(V_x_matrix.^2+V_x_matrix.^2)))
%set(h,'edgecolor','none','facecolor','interp');
% colorbar
%  quiver(avg(y),avg(x),(Ve./Len),(Ue./Len),.4,'y-')       
%title('')  
%axis equal, axis([0 ly 0 lx])
%box on
